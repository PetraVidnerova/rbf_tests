...
if __name__ == "__main__":
    ...
    open("{}.json".format(name),"w").write(json_string) 
    newmodel.save_weights("{}_weights.h5".format(name))

    print("---test----")
    m = model_from_json(open("{}.json".format(name)).read(), {'RBFLayer':RBFLayer})
    m.load_weights("{}_weights.h5".format(name))
